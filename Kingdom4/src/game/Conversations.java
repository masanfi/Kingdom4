        ...
        if (trigger.getName().contentEquals("lady")) {
            if(!this.getStatusSpeechBubble()) {
                if (gameEngine.getCharacter().get("lady") == 0) {
                	...
                	timeline.getKeyFrames().add(new KeyFrame(Duration.millis(0), ae -> this.showSpeechBubble(triggerX, triggerY, "Du bist ja immer\nnoch hier!", 2.5, Color.RED, Color.WHITE)));
                    ...
                    timeline.play();
                    gameEngine.getCharacter().put("lady", 3);
                }
                else if (gameEngine.getCharacter().get("lady") == 3) {
                    timeline.getKeyFrames().add(new KeyFrame(Duration.millis(0), ae -> this.showSpeechBubble(triggerX, triggerY, "Ist dir das\nnicht peinlich?", 3, Color.RED, Color.WHITE)));
                    timeline.play();
                }
            ...
            if(!this.getStatusSpeechBubble()) {
                if (gameEngine.getCharacter().get("blacksmith") == 0 && gameEngine.getCharacter().get("knight") == 0) {
                    ...
                    if(gameEngine.findItemInInventory("R")) {
                        ...
                        gameEngine.getCharacter().put("blacksmith", 2);
                    }
                    else {
                    ...
                    timeline.getKeyFrames().add(new KeyFrame(Duration.millis(0), ae -> this.showSpeechBubble(triggerX, triggerY, "Ich produziere gerade\nfür einen anderen Kunden,\nich hab keine Zeit.", 3.5, Color.DARKRED, Color.WHITE)));
                    timeline.play();
                }            
                else if (gameEngine.getCharacter().get("blacksmith") == 2 && gameEngine.getCharacter().get("knight") == 2){
                    ...
                    timeline.getKeyFrames().add(new KeyFrame(Duration.millis(0), ae -> this.showSpeechBubble(triggerX, triggerY, "Ich hab zu tun.\nDer weise Mann kann\ndir aber unendlich viel\nerzählen!", 3.5, Color.DARKRED, Color.WHITE)));
                    timeline.play();
                }
                this.setStatusSpeechBubble(true);
            }
            ...
            if(!this.getStatusSpeechBubble()) {
                if (gameEngine.getCharacter().get("fisherman") == 0 && gameEngine.getCharacter().get("blacksmith") < 2) {
                ...
                else if (gameEngine.getCharacter().get("fisherman") == 0 && gameEngine.getCharacter().get("blacksmith") >= 2) {
                	timeline.getKeyFrames().add(new KeyFrame(Duration.millis(0), ae -> this.showSpeechBubble(playerX, playerY, "*hust* ...ähm ... Hallo!", 2, Color.WHITE, Color.BLACK)));
                    ...
                    timeline.getKeyFrames().add(new KeyFrame(Duration.millis(2100), ae -> this.showSpeechBubble(triggerX, triggerY, "So lange du nicht weißt\nwas die Bedeutung von\n42 ist, brauchst du\nnicht wiederkommen.", 4, Color.YELLOW, Color.BLACK)));
                    timeline.play();
                }
                this.setStatusSpeechBubble(true);
            }
